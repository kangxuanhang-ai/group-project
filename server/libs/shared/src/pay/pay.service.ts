...
export class PayService implements OnModuleInit {
    ...
    async createPay(createPayDto: CreatePayDto, userId: string): Promise<PayResponse> {
        // 检查是否已购买
        ...
        // 创建支付宝支付请求
        const result = await this.alipaySdk.pageExec('alipay.trade.page.pay', orderParams);
    ...
    async queryPayStatus(orderId: string) {
        ...
        const alipayStatus = response.alipay_trade_query_response?.trade_status || '';
